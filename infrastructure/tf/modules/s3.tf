...
resource "aws_s3_bucket" "staged_data" {
  bucket = "${var.project_name}-staged-data-${data.aws_caller_identity.current.account_id}"
}

resource "aws_s3_bucket_public_access_block" "staged_data" {
...
resource "aws_s3_bucket_lifecycle_configuration" "staged_data" {
  bucket = aws_s3_bucket.staged_data.id
